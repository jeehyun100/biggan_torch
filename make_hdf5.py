...
def prepare_parser():
  ...
  parser.add_argument(
    '--dataset', type=str, default='D256', #D256
    help='Which Dataset to train on, out of I128, I256, C10, C100;'
         'Append "_hdf5" to use the hdf5 version for ISLVRC (default: %(default)s)')
  parser.add_argument(
    '--data_root', type=str, default='/home/dev/diade/train',
...
def run(config):
  ...
  for i,(x,y) in enumerate(tqdm(train_loader)):
    ...
    if i==0:
      with h5.File(config['data_root'] + '/DIODE%i.hdf5' % config['image_size'], 'w') as f:
    ...
    # Else append to the hdf5
      ...
      with h5.File(config['data_root'] + '/DIODE%i.hdf5' % config['image_size'], 'a') as f:
